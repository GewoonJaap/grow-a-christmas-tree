...
export class SetTimezone implements ISlashCommand {
  ...
  public handler = async (ctx: SlashCommandContext): Promise<void> => {
    if (ctx.isDM) {
      ...
      return await safeReply(ctx, new MessageBuilder().addEmbed(embed));
    }
    if (!ctx.game) {
      const embed = new EmbedBuilder()
        .setTitle("Woah there!")
    ...
    if (!perms.includes("MANAGE_GUILD"))
      return await safeReply(
        ctx,
        new MessageBuilder().setContent(`You need the Manage Server permission to set the time zone for your server.`)
      );
